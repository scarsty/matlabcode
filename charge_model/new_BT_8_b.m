...
% elem{2} = struct('elem',{'elpconstr'},'g',{{'1'}},'geomdim',{{{struct('constr',{{{'c2-(Dx)','c1-(D+sigma/P0)'}}},'cpoints',{{{'2','2'}}},'ind',{{{'2'}}}),{}}}});
% fem.elemcpl = elem;
...
count=1;
while stop~=0 & count~=4
    fem0=fem;
    fem.sol=femnlin(fem, ...
        'init',fem.sol, ...
        'solcomp',{'D'}, ...
        'outcomp',{'D'});
    stop=femnlin(fem,'init',fem.sol,'Out','stop');
    count=count+1;
end

if stop==0
    display('be convergent.');
else
    display('not be convergent.not be convergent.not be convergent.not be convergent.not be convergent.');
end
